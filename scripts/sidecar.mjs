 ...
 * `prism-cli` with the platform's executable suffix and Windows will not run a file without one.
 * One file in a directory of its own, so the bundle can take the directory without naming what
 * is in it.
 */
...
/** What the client is called on this platform. */
const name = `prism-cli${process.platform === 'win32' ? '.exe' : ''}`;

/**
 * The features the client needs to be worth shipping.
 *
...
execFileSync(
  'cargo',
  ['build', '--release', '--locked', '-p', 'prism-cli', '--features', FEATURES],
