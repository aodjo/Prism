...
//!
//! Three programs have to reach the same answer from the same password: the application, the
//! account server that checks the verifier, and the operator dashboard, which derives in a
//! browser through `prism-account-wasm`. A second implementation would be a second answer, and
//! it would show up as an account that signs in on one and not the other.
//!
//! Nothing here touches the operating system, the network, or randomness, which is what lets it
//! compile for `wasm32-unknown-unknown` unchanged. Generating a salt needs randomness and so
//! lives in `prism_core::account::secret` instead.
//!
...
pub enum SecretError {
    ...
    /// one — see `prism_core::account` and the account server's own errors for the same reason.
...
mod tests {
    ...
    /// What `derive` must answer for a fixed password and salt, as hex.
    ///
    /// Pinned rather than merely self-consistent, because three programs derive this and a
    /// change here would not fail any test that only compares one run against another. Every
    /// account that exists was created against this value: changing it locks everybody out.
    const KNOWN_AUTH: &str = "75b9e2e88077c9b697b6a4a861cb85b79cde04c3cf65bc4b3c1ff3324fd4b262";
    ...
    #[test]
    fn the_authentication_secret_is_the_value_every_account_was_made_against() {
        // The one test the browser's copy and the application's copy both have to pass. It
        // fails if a constant above is edited, which is the only way the three could disagree.
        let secrets = derive("correct horse battery", &SALT).expect("derives");
        let hex: String = secrets.auth.iter().map(|b| format!("{b:02x}")).collect();

        assert_eq!(hex, KNOWN_AUTH);
    }
}
