...
fn the_first_frame_anchors_the_report_with_an_empty_history() {
    ...
    let report = tracker.report(1000, 0).expect("a frame has arrived");
...
#[test]
fn the_flags_the_caller_passes_reach_the_report_unchanged() {
    // The tracker counts frames; what the client wants doing about them is decided
    // elsewhere. Rewriting the flag here would be a silent way to lose a keyframe request.
    let mut tracker = AckTracker::new();
    tracker.received(1);

    assert_eq!(tracker.report(0, 0).expect("a frame arrived").flags, 0);
    assert_eq!(
        tracker
            .report(0, FEEDBACK_WANTS_KEYFRAME)
